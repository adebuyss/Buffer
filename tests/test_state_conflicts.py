...
from conftest import (
    FORWARD, BACK, STOP,
    STATE_IDLE, STATE_STOPPED, STATE_DISABLED, STATE_ERROR,
    STATE_FEEDING, STATE_MANUAL_FEED, STATE_MANUAL_RETRACT,
    MockGcmd, set_sensors,
)
...
class TestResetConsolidation:
    def _dirty(self, buf):
        ...
        buf._burst_count = 7
        buf._velocity_window = [(0., 1.)]
        buf._manual_feed_full_start = 1.0

    def test_reset_control_state_clears_all(self, buf):
        self._dirty(buf)
        buf._reset_control_state()
        assert buf._full_zone_feed_time == 0.
        assert buf._last_full_feed_time == 0.
        assert buf._full_zone_retract_start == 0.
        assert buf._burst_delay_start == 0.
        assert buf._burst_until == 0.
        assert buf._burst_count == 0
        assert buf._velocity_window == []
        assert buf._manual_feed_full_start == 0.

    def test_enable_uses_reset_control_state(self, buf):
        buf._full_zone_retract_start = 99.
        buf.cmd_BUFFER_ENABLE(MockGcmd('BUFFER_ENABLE'))
        assert buf._full_zone_retract_start == 0.
